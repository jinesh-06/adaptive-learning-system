...
(course, topic, level), configurable top-k retrieval, and relevance threshold filtering.
"""
...
def retrieve_documents(
    ...
    Args:
        ...
        level: Optional difficulty level filter ("beginner", "intermediate", "advanced").
        threshold: Optional minimum cosine similarity threshold (e.g. 0.40).
    ...
    fetch_k = top_k * 3 if threshold is not None else top_k
    try:
    ...
    for doc_text, meta, dist, cid in zip(documents_list, metadatas_list, distances_list, ids_list):
        ...
        retrieved.append({
            ...
            "level": meta_dict.get("level", "intermediate"),
...
if __name__ == "__main__":
    ...
    results = retrieve_documents(test_q, top_k=3, course="c")
    for i, r in enumerate(results, 1):
        print(f"\n[{i}] ID: {r['chunk_id']} | Source: {r['source']} | Course: {r['course']} | Sim: {r['similarity']:.3f}")
        print(f"Content:\n{r['text'][:150]}...")
