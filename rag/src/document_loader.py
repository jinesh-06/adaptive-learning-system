...
def normalize_course_name(raw_name: str) -> str:
    """
    Normalize course/language name to standard identifiers: 'c', 'cpp', 'python', 'java'.
    """
    cleaned = raw_name.strip().lower()
    if "cpp" in cleaned or "c++" in cleaned or "cplusplus" in cleaned:
        return "cpp"
    elif "python" in cleaned:
        return "python"
    elif "java" in cleaned and "javascript" not in cleaned:
        return "java"
    elif cleaned == "c" or cleaned.startswith("c ") or cleaned.startswith("c_") or " c " in f" {cleaned} ":
        return "c"
    return cleaned
...
def load_documents(
    ...
    for file_path in txt_files:
        try:
            ...
            raw_level = (doc_meta.get("level") or path_meta["level"]).lower().strip()
            if "beginner to advanced" in raw_level or "all" in raw_level:
                level = path_meta["level"]
            elif "beginner" in raw_level:
                level = "beginner"
